...
\begin{abstract}
Standard RAG evaluation assumes that improving retrieval quality
directly improves end-task accuracy---an assumption we show fails
for multi-hop reasoning.
We identify the \emph{retrieval--reasoning gap}: retrieval-side
metric gains do not transfer to reasoning accuracy when the
language model is held constant.
Using a six-configuration ablation on 65 multi-hop queries
(synthetic telecom logs, 1{,}115 entries), we find Precision@K
varies by 0.24 ($p{<}0.001$) while root-cause accuracy remains
statistically flat ($\Delta{=}0.05$, $p{=}0.08$, n.s.;
Pearson $r{=}0.244$).
A multi-LLM comparison (8B--70B) with fixed retrieval confirms
reasoning is independently bounded by model capability.
We propose a composite metric (semantic similarity + ROUGE-L +
structured scoring) that captures diagnostic quality beyond
keyword overlap ($r{=}0.849$ with semantic evaluation).
These findings generalize beyond text-only pipelines: any
multimodal retrieval system that assembles heterogeneous evidence
before generation faces the same structural gap between
...
This work challenges that core assumption with controlled empirical
evidence: improving retrieval alone is insufficient for multi-hop
reasoning when the reader model is fixed.

This paper presents the \textbf{retrieval--reasoning gap}: the
observation that improvements in retrieval-side metrics do not
translate to improvements in reasoning accuracy, and that this
gap is a structural property of multi-hop RAG rather than an artifact
of dataset size or domain.
...
Although our experiments are text-based, the retrieval--reasoning gap
is not modality-specific.
Multimodal retrieval systems that integrate image+text, video+text,
or structured data face the same structural challenge: retrieving
relevant heterogeneous evidence does not guarantee that a downstream
multimodal generation model will reason correctly over it.
Our evaluation framework---decomposing retrieval from reasoning---applies
directly to such systems.

\paragraph{Contributions.}
\begin{enumerate}[nosep]
    \item \textbf{Identifying the retrieval--reasoning gap as a
    structural property.}
    ...
    alone.
    \item \textbf{A decomposition protocol for multi-hop RAG evaluation.}
    We propose: (a)~ablation with fixed LLM, (b)~model comparison
    with fixed retrieval, (c)~per-query regime classification
    (high/low retrieval $\times$ correct/incorrect reasoning)---a
    framework applicable to both text-only and multimodal pipelines.
...
The retriever uses \texttt{all-MiniLM-L6-v2} (22M parameters, 384-dim
embeddings) indexed in ChromaDB (in-memory).
The cross-encoder reranker is \texttt{ms-marco-MiniLM-L-6-v2} (22M
parameters, trained on MS~MARCO passages).
The default LLM is Llama~3.3~70B served via Groq (temperature~0,
\texttt{max\_tokens=2048}).
Indexing is performed per-session; inference runs on CPU for
embedding and reranking, with LLM calls via API.
All models are used off-the-shelf without domain fine-tuning.
...
\paragraph{Dataset Justification.}
The corpus is synthetic by design.
Production telecom logs contain sensitive subscriber data, proprietary
protocol traces, and operator-specific configurations that preclude
public release under standard security and privacy constraints.
A synthetic corpus offers three advantages for our study:
(i)~complete control over failure complexity and causal chain length,
enabling systematic evaluation;
(ii)~known ground-truth root causes for every query, eliminating
annotation ambiguity;
(iii)~full reproducibility, as both data and evaluation code are
publicly released.
...
Scatter plots of retrieval score vs.\ reasoning accuracy for all
390 query$\times$config pairs are included in the supplementary
material.
...
\subsection{Implications for Multimodal Systems}

The retrieval--reasoning gap is likely amplified in multimodal
settings.
When a system retrieves heterogeneous evidence---images, tables,
video segments, and text---from multiple sources, improving retrieval
recall for each modality does not ensure the generation model can
integrate cross-modal evidence into coherent reasoning.
The challenge of assembling and reasoning over disparate evidence
types mirrors the multi-hop text problem we study, but adds
modality-alignment as an additional failure mode.
Our decomposition framework (fixed retrieval $\times$ varied model,
and vice versa) offers a principled evaluation template for
multimodal generation pipelines facing this gap.
...
All experiments use text-based retrieval and generation; extending
to multimodal RAG (image+text, video+text) is necessary to confirm
whether the retrieval--reasoning gap persists---or widens---across
modalities.

\textbf{Synthetic dataset.}
The corpus is synthetically generated with known ground truths.
While this enables controlled evaluation, it may not capture the
noise distributions, annotation ambiguity, and scale of production
systems.
Validation on real-world telecom logs and established multi-hop
benchmarks (HotpotQA, MuSiQue) is needed to confirm generalizability.

\textbf{LLM-as-judge bias.}
Cross-model judges and human evaluation
are needed to validate automated scoring.

\textbf{Single domain.}
Replication on additional domains (code-assistant, scientific QA,
medical) would strengthen generality claims.

\textbf{Confidence calibration.}
The composite confidence score is a heuristic,
not a formally calibrated metric.

Future directions include: (i)~multimodal retrieval--reasoning
experiments with image+text evidence; (ii)~larger corpora with
bootstrap significance testing; (iii)~human-calibrated evaluation;
(iv)~domain-adapted embeddings via contrastive learning;
(v)~graph-based causal retrieval; (vi)~reader fine-tuning to test
whether reasoning improvements close the gap.
